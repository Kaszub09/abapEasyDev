...
cd->change_multi( after = cd->create_table_with_indicator( REF #( inserted ) )
...
"Sort tables if they aren't sorted yet
"Should detect by keys what was changed and what not
cd->change_multi( before = cd->create_table_with_indicator( original_table = REF #( old ) sort = abap_true )
    after = cd->create_table_with_indicator( original_table = REF #( new ) )
    force_cd_on_all_fields = abap_true
