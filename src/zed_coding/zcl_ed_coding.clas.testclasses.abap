...
CLASS ltcl_ed_coding IMPLEMENTATION.
  METHOD convert_system_to_utf8.
    DATA(result) = zcl_ed_coding=>convert( input = test_string in_codepage = zcl_ed_coding=>as_codepage out_codepage = zcl_ed_coding=>c_utf8 ).
  ...
  METHOD convert_utf8_to_system.
    ...
    DATA(result) = cl_bcs_convert=>xstring_to_string( iv_xstr = test_xstring_utf8 iv_cp = zcl_ed_coding=>c_utf8 ).
    cl_abap_unit_assert=>assert_equals( act = result exp = test_string ).
  ENDMETHOD.

  METHOD no_conversion_between_same_cp.
    DATA(result) = zcl_ed_coding=>convert( input = test_string in_codepage = zcl_ed_coding=>c_utf8 out_codepage = zcl_ed_coding=>c_utf8 ).
  ...
  METHOD convert_utf8_to_utf16le.
    DATA(test_xstring_utf8) = cl_bcs_convert=>string_to_xstring( iv_string = test_string iv_convert_cp = abap_true
                                                                 iv_codepage = CONV #( zcl_ed_coding=>c_utf8 ) ).
    DATA(text_string_utf8) = cl_bcs_convert=>xstring_to_string( iv_xstr = test_xstring_utf8 iv_cp = zcl_ed_coding=>c_utf16le ).
    DATA(text_string_utf16le) = zcl_ed_coding=>convert( input = text_string_utf8 in_codepage = zcl_ed_coding=>c_utf8
                                                        out_codepage = zcl_ed_coding=>c_utf16le ).
    DATA(text_xstring_utf16le) = cl_bcs_convert=>string_to_xstring( iv_string = text_string_utf16le iv_convert_cp = abap_true
                                                                 iv_codepage = CONV #( zcl_ed_coding=>c_utf16le ) ).
